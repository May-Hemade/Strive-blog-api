...
export const basicAuthMiddleware = async (req, res, next) => {
  ...
  if (!req.headers.authorization) {
    ...
    if (author) {
      ...
      req.user = author
      // Adding the current author to the req object is going to unlock a number of possibilities like using some subsequent middlewares to check the role of that user for instance (Authorization)
      next()
    } else {
