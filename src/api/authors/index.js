...
authorsRouter.post("/:id/avatar", parseFile.single("avatar"), uploadAuthorAvatar, async (req, res, next) => {
  try {
    let authorsArray = await getAuthors()

    let authorIndex = authorsArray.findIndex((author) => author.ID === req.params.id)
    if (authorIndex !== -1) {
